...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Larry
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Larry
    // Mac: ~/Library/Application Support/Larry
    // Unix: ~/.Larry
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Larry";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Larry.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) //Larry.conf doesn't exist
    {
        createConf();
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Larryd.pid"));
