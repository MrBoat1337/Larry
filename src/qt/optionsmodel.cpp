...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeLarryAmount"))
        settings.setValue("nAnonymizeLarryAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeLarryAmount = settings.value("nAnonymizeLarryAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeLarryAmount"))
        SoftSetArg("-anonymizeLarryamount", settings.value("nAnonymizeLarryAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeLarryAmount:
            nAnonymizeLarryAmount = value.toInt();
            settings.setValue("nAnonymizeLarryAmount", nAnonymizeLarryAmount);
            emit anonymizeLarryAmountChanged(nAnonymizeLarryAmount);
            break;
        default:
            break;
        }
