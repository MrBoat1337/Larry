...
{
    // Convert Larry:// to Larry:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Larry://"))
    {
        uri.replace(0, 12, "Larry:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Larry.lnk";
}
...
{
    return GetAutostartDir() / "Larry.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Larry-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Larry-qt [" + tr("command-line options") + "]                     " + "\n";
