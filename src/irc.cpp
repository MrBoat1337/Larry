...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #LarryTEST\r");
            Send(hSocket, "WHO #LarryTEST\r");
        } else
        {
	    LogPrintf("JOIN #Larry\n");
            // randomly join #Larry00-#Larry05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Larry%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Larry%02d\r", channel_number).c_str());
        };
